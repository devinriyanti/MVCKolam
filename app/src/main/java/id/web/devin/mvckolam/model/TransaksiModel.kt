...
class TransaksiModel(private val context: Context) {
    interface TransaksiCallback {
        fun onTransaksiLoaded(transaksiList: List<Transaction>)
        ...
        val stringReq = object : StringRequest(
            ...
            Response.Listener { response ->
                ...
                if(status.equals("success")){
                    ...
                    callback.onError("Gagal Menambah Transaksi")
    ...
    fun fetchTransaksi(email:String, status:String,callback: TransaksiCallback) {
        queue = Volley.newRequestQueue(context)
        ...
        val stringReq = object : StringRequest(
            ...
            { response ->
                ...
                val transaksi = parseTransaksi(response,callback)
                Log.d("successTransaksi", transaksi.toString())
                callback.onTransaksiLoaded(transaksi)
            },
            { error ->
    ...
    fun fetchTransaksiAdmin(email:String, status:String,callback: TransaksiCallback) {
        queue = Volley.newRequestQueue(context)
    ...
    fun updateStatus(idtransaksi: String, status: String,callback: TransaksiCallback){
        queue = Volley.newRequestQueue(context)
    ...
    fun konfirmasiTransaksi(idtransaksi: String, status: String, no_resi:String,callback: TransaksiCallback){
        queue = Volley.newRequestQueue(context)
    ...
    fun updateBuktiPembayaran(urlBukti:String, idtransaksi: String,callback: TransaksiCallback){
        queue = Volley.newRequestQueue(context)
    ...
    private fun parseTransaksi(response: String?,callback: TransaksiCallback): List<Transaction> {
        ...
        try {
            ...
            callback.onError("Tidak Ada Transaksi")
        }
        return transactions
    }
