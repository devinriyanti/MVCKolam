...
class KolamModel(private val context: Context) {
    interface KolamCallback {
        fun onKolamLoaded(kolamList: List<Kolam>)
    ...
    fun fetchKolam(cari:String, callback: KolamCallback) {
        queue = Volley.newRequestQueue(context)
        val url = "https://lokowai.shop/kolam.php?cari=$cari"
        val stringReq = StringRequest(
            Request.Method.GET, url,
            {response->
                val kolam = parseKolam(response,callback)
                Log.d("successKolam", kolam.toString())
                callback.onKolamLoaded(kolam)
            },
            {error->
    ...
    fun fetchKolamAdmin(email:String, role:String,callback: KolamCallback){
        queue = Volley.newRequestQueue(context)
        ...
        val stringReq = object : StringRequest(
            ...
            Response.Listener{ response ->
                ...
                if(status.equals("success")){
                    ...
                    callback.onError("Gagal menambah kolam")
        ...
        val stringReq = object  : StringRequest(
            ...
            Response.Listener {
                ...
                if(status.equals("success")){
                    ...
                    callback.onError("Gagal Mengubah Data Kolam")
    ...
    fun removeKolam(idkolam:String,callback: KolamCallback){
        queue = Volley.newRequestQueue(context)
    ...
    fun updateMaintenance(status:Int, idkolam: String,callback: KolamCallback){
        queue = Volley.newRequestQueue(context)
        ...
        val stringReq = object  : StringRequest(
            ...
            Response.Listener {
                ...
                if(status.equals("success")){
                    ...
                    Log.d("showError",it.toString())
                    callback.onError("Gagal Mengubah Status Kolam")
                }
            },
            Response.ErrorListener {
                Toast.makeText(context,"Kesalahan Saat Mengakses Basis Data", Toast.LENGTH_SHORT).show()
    ...
    fun updateStatus(status:Int, idkolam: String,callback: KolamCallback){
        queue = Volley.newRequestQueue(context)
    ...
    private fun parseKolam(response: String?,callback: KolamCallback): List<Kolam> {
        ...
        try {
            ...
            for (i in 0 until jsonArray.length()) {
                ...
                val pelatihList = parsePelatihList(pelatihArray,callback)
            ...
            callback.onError("Tidak Ada Kolam")
        }
        return kolams
    }

    private fun parsePelatihList(array: JSONArray, callback: KolamCallback): List<Pelatih> {
        ...
        try{
            ...
            // Handle JSON parsing error
           callback.onError("Error parsing pelatih")
        }
        return pelatihList
    }
